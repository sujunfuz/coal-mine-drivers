...
export default {
	data() {
		return {
			...
			plateType:uni.getStorageSync('userInfo').plateType
		}
	},
	onLoad() {
		// #ifndef H5
	...
	methods: {
		...
		handleCardNo(str) {
			if (str) {
				const start = str.substr(0, 4);
				const center = str.substr(4, 2);
				const end = str.substr(-4, 4)
				const result = start + ' ' + center + '** **** ' + end
				return result
			}
		},
		//截取后4位
		...
		//判断是否开发中
		checkDeving(url, islogin) {
			if (!url) {
				this.showMsg('暂未开放，敬请期待')
			} else if (islogin&&!this.isLogin) {
		...
		wxlogin(code, type) {
			...
			let obj = {
				...
				// wxAvatar: this.avatarUrl,
				// wxNick: this.nickName,
				code: code
			}
			commonApi.wxlogin(obj).then(res => {
				uni.hideLoading()
				if (res.code == 200) {
					this.isLogin = true;
		...
		async isGetBasic() {
			let iscall = false;
			if (!uni.getStorageSync('userNo') || !uni.getStorageSync('openId')) {
				iscall = true
			} else {
				iscall = false
			}
